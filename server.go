...
// broadcastMsg provides the ability to house a abcd message to be broadcast
...
// server provides a abcd server for handling communications to and from
// abcd peers.
type server struct {
...
func (sp *serverPeer) OnTx(p *peer.Peer, msg *wire.MsgTx) {
	...
	// Convert the raw MsgTx to a abcutil.Tx which provides some convenience
	// methods and things such as hash caching.
	tx := abcutil.NewTx(msg)
...
func (sp *serverPeer) OnBlock(p *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	// Convert the raw MsgBlock to a abcutil.Block which provides some
	// convenience methods and things such as hash caching.
	block := abcutil.NewBlockFromBlockAndBytes(msg, buf)
...
func (s *server) AnnounceNewTransactions(newTxs []*abcutil.Tx) {
...
func (s *server) peerHandler() {
	...
	if !cfg.DisableDNSSeed {
		...
		connmgr.SeedFromDNS(activeNetParams.Params, abcdLookup, func(addrs []*wire.NetAddress) {
...
// AddedNodeInfo returns an array of abcjson.GetAddedNodeInfoResult structures
...
// newServer returns a new abcd server configured to listen on addr for the
// abcd network type specified by chainParams.  Use start to begin accepting
...
func newServer(listenAddrs []string, db database.DB, chainParams *chaincfg.Params) (*server, error) {
	...
	amgr := addrmgr.New(cfg.DataDir, abcdLookup)
...
func addrStringToNetAddr(addr string) (net.Addr, error) {
	...
	// The abcdLookup function will transparently handle performing the
	// lookup over Tor if necessary.
	ips, err := abcdLookup(host)
	if err != nil {
		return nil, err
	}
	if len(ips) == 0 {
