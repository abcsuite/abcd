...
func containsTxIns(txs []*abcutil.Tx, tx *abcutil.Tx) bool {
...
// can not be found.
func txIndexFromTxList(hash chainhash.Hash, list []*abcutil.Tx) int {
	for i, tx := range list {
		h := tx.Hash()
		if hash == *h {
			return i
...
func UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int64,
	...
	// TODO(davec): A abcutil.Block should use saved in the state to avoid
	// recalculating all of the other transaction hashes.
	// block.Transactions[0].InvalidateCache()

	// Recalculate the merkle root with the updated extra nonce.
	block := abcutil.NewBlockDeepCopyCoinbase(msgBlock)
...
func createCoinbaseTx(subsidyCache *blockchain.SubsidyCache,
	...
	voters uint16,
	params *chaincfg.Params) (*abcutil.Tx, error) {
	...
	if nextBlockHeight == 1 && len(params.BlockOneLedger) != 0 {
		...
		addrs := make([]abcutil.Address, len(params.BlockOneLedger))
		for i, payout := range params.BlockOneLedger {
			addr, err := abcutil.DecodeAddress(payout.Address, params)
			if err != nil {
				return nil, err
			}
			addrs[i] = addr
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *abcutil.Tx,
	height int64) error {
	for _, txIn := range tx.MsgTx().TxIn {
...
func logSkippedDeps(tx *abcutil.Tx, deps *list.List) {
	if deps == nil {
		return
	}
...
func maybeInsertStakeTx(bm *blockManager, stx *abcutil.Tx, treeValid bool) bool {
	missingInput := false
...
func deepCopyBlockTemplate(blockTemplate *BlockTemplate) *BlockTemplate {
	...
	coinbaseCopy :=
		abcutil.NewTxDeep(blockTemplate.Block.Transactions[0])
	...
	if nextHeight >= stakeValidationHeight {
		if bm.AggressiveMining {
			if curTemplate != nil {
				...
				block := abcutil.NewBlockDeepCopyCoinbase(cptCopy.Block)
			...
			if curTemplate == nil {
				...
				btBlockTemp := abcutil.NewBlockDeepCopyCoinbase(btMsgBlock)
				merkles :=
				...
				// Make sure the block validates.
				btBlock := abcutil.NewBlockDeepCopyCoinbase(btMsgBlock)
...
func NewBlockTemplate(policy *mining.Policy, server *server,
	payToAddress abcutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*abcutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		// Consider caching this in the mempool in the future. - Aero
	...
	blockTxnsStake := make([]*abcutil.Tx, 0, len(blockTxns))
	...
	for _, tx := range blockTxns {
		...
		if isSSGen, _ := stake.IsSSGen(msgTx); isSSGen {
			txCopy := abcutil.NewTxDeepTxIns(msgTx)
	...
	if nextBlockHeight < stakeValidationHeight {
		...
		for _, vb := range voteBitsVoters {
			if abcutil.IsFlagSet16(vb, abcutil.BlockValid) {
				voteYea++
			}
			totalVotes++
		}
		...
		if votebits == uint16(0x0000) {
			...
			// the previous block.
			// Aero WARNING: The ideal behaviour should also be that we re-add
			...
			// out.
			// Aero TODO: This is super inefficient, this block should be
			...
			tempBlockTxns := make([]*abcutil.Tx, 0, len(sourceTxns))
			for _, tx := range blockTxns {
				if tx.Tree() == wire.TxTreeRegular {
					...
					if isValid {
						txCopy := abcutil.NewTxDeepTxIns(tx.MsgTx())
	...
	// Aero: We need to move this downwards because of the requirements
	...
	blockTxnsRegular := make([]*abcutil.Tx, 0, len(blockTxns)+1)
	...
	minrLog.Debugf("Created new block template (%d transactions, %d "+
		...
		abcutil.Amount(msgBlock.Header.SBits).ToCoin())
