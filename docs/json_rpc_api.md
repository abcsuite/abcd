...
abcd provides a [JSON-RPC](http://json-rpc.org/wiki/specification) API that is
fully compatible with the original bitcoind/bitcoin-qt.  There are a few key
differences between abcd and bitcoind as far as how RPCs are serviced:
* Unlike bitcoind that has the wallet and chain intermingled in the same process
  which leads to several issues, abcd intentionally splits the wallet and chain
  ...
  talking directly to abcd, only chain-related RPCs are available.  However both
  chain-related and wallet-related RPCs are available via
  [Abcwallet](https://github.com/abcsuite/abcd).
* abcd is secure by default which means that the RPC connection is TLS-enabled
  by default
* abcd provides access to the API through both
  [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29) requests and
  [Websockets](http://en.wikipedia.org/wiki/WebSocket)

Websockets are the preferred transport for abcd RPC and are used by applications
such as [Abcwallet](https://github.com/abcsuite/abcd) for inter-process
communication with abcd.  The websocket connection endpoint for abcd is
`wss://your_ip_or_domain:9528/ws`.
...
during the development of Abcwallet.
...
The abcd RPC server supports both [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29)
...
abcd is `wss://your_ip_or_domain:9528/ws`.
...
to a abcd RPC server:

* **rpcuser** is the full-access username configured for the abcd RPC server
* **rpcpass** is the full-access password configured for the abcd RPC server
* **rpclimituser** is the limited username configured for the abcd RPC server
* **rpclimitpass** is the limited password configured for the abcd RPC server
* **rpccert** is the PEM-encoded X.509 certificate (public key) that the abcd
  server is configured with.  It is automatically generated by abcd and placed
  in the abcd home directory (which is typically `%LOCALAPPDATA%\abcd` on
  Windows and `~/.abcd` on POSIX-like OSes)

**NOTE:** As mentioned above, abcd is secure by default which means the RPC
...
The abcd RPC server uses HTTP [basic access authentication](http://en.wikipedia.org/wiki/Basic_access_authentication) with the **rpcuser**
...
abcd comes with a separate utility named `abcctl` which can be used to issue
these RPC commands via HTTP POST requests to abcd after configuring it with the
...
|23|[getwork](#getwork)|N|Returns formatted hash data to work on or checks and submits solved data.<br /><font color="orange">NOTE: Since abcd does not have the wallet integrated to provide payment addresses, abcd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.</font>|
...
|26|[sendrawtransaction](#sendrawtransaction)|Y|Submits the serialized, hex-encoded transaction to the local peer and relays it to the network.<br /><font color="orange">abcd does not yet implement the `allowhighfees` parameter, so it has no effect</font>|
|27|[setgenerate](#setgenerate) |N|Set the server to generate coins (mine) or not.<br/>NOTE: Since abcd does not have the wallet integrated to provide payment addresses, abcd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|28|[stop](#stop)|N|Shutdown abcd.|
|29|[submitblock](#submitblock)|Y|Attempts to submit a new serialized, hex-encoded block to the network.|
|30|[validateaddress](#validateaddress)|Y|Verifies the given address is valid.  NOTE: Since abcd does not have a wallet integrated, abcd will only return whether the address is valid or not.|
...
|Example Return (dns=false)|`["192.168.0.10:9527", "mydomain.org:9527"]`|
|Example Return (dns=true)|`[`<br />&nbsp;&nbsp;`{`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"addednode": "mydomain.org:9527",`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"connected": true,`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"addresses": [`<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`{`<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`"address": "1.2.3.4",`<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`"connected": "outbound"`<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`},`<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`{`<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`"address": "5.6.7.8",`<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`"connected": "false"`<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`}`<br />&nbsp;&nbsp;&nbsp;&nbsp;`]`<br />&nbsp;&nbsp;`}`<br />`]`|
[Return to Overview](#MethodOverview)<br />
...
|---|---|
|Method|getblock|
|Parameters|1. block hash (string, required) - the hash of the block<br />2. verbose (boolean, optional, default=true) - specifies the block is returned as a JSON object instead of hex-encoded string<br />3. verbosetx (boolean, optional, default=false) - specifies that each transaction is returned as a JSON object and only applies if the `verbose` flag is true.<font color="orange">**This parameter is a abcd extension**</font>|
...
|Example Return|`[`<br />&nbsp;&nbsp;`{`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"addr": "178.172.xxx.xxx:9527",`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"services": "00000001",`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"lastrecv": 1388183523,`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"lastsend": 1388185470,`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"bytessent": 287592965,`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"bytesrecv": 780340,`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"conntime": 1388182973,`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"pingtime": 405551,`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"pingwait": 183023,`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"version": 70001,`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"subver": "/abcd:0.4.0/",`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"inbound": false,`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"startingheight": 276921,`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"currentheight": 276955,`<br/>&nbsp;&nbsp;&nbsp;&nbsp;`"syncnode": true,`<br />&nbsp;&nbsp;`}`<br />`]`|
[Return to Overview](#MethodOverview)<br />
...
|Description|Returns information about a transaction given its hash.|
|Notes|<font color="orange">NOTE: Since abcd does not have the wallet integrated to provide payment addresses, abcd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.</font>
...
|Notes|NOTE: Since abcd does not have the wallet integrated to provide payment addresses, abcd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|Returns|Nothing|
[Return to Overview](#MethodOverview)<br />
...
|Notes|<font color="orange">abcd does not yet implement the `allowhighfees` parameter, so it has no effect</font>|
...
|Method|stop|
|Parameters|None|
|Description|Shutdown abcd.|
|Returns|`"abcd stopping."` (string)|
[Return to Overview](#MethodOverview)<br />
...
|Description|Verifies the block chain database.<br />The actual checks performed by the `checklevel` parameter is implementation specific.  For abcd this is:<br />`checklevel=0` - Look up each block and ensure it can be loaded from the database.<br />`checklevel=1` - Perform basic context-free sanity checks on each block.|
|Notes|<font color="orange">abcd currently only supports `checklevel` 0 and 1, but the default is still 3 for compatibility.  Per the information in the Parameters section above, higher levels are automatically clamped to the highest supported level, so this means the default is effectively 1 for abcd.</font>|
...
|3|[getcurrentnet](#getcurrentnet)|Y|Get abcd network abcd is running on.|None|
...
|Description|Dynamically changes the debug logging level.<br />The levelspec can either a debug level or of the form `<subsystem>=<level>,<subsystem2>=<level2>,...`<br />The valid debug levels are `trace`, `debug`, `info`, `warn`, `error`, and `critical`.<br />The valid subsystems are `AMGR`, `ADXR`, `BCDB`, `BMGR`, `ABCD`, `CHAN`, `DISC`, `PEER`, `RPCS`, `SCRP`, `SRVR`, and `TXMP`.<br />Additionally, the special keyword `show` can be used to get a list of the available subsystems.|
|Returns|string|
|Example Return|`Done.`|
|Example `show` Return|`Supported subsystems [AMGR ADXR BCDB BMGR ABCD CHAN DISC PEER RPCS SCRP SRVR TXMP]`|
...
|Parameters|None|
|Description|Get abcd network abcd is running on.|
|Returns|numeric|
...
|Description|Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this abcd instance) and when such a transaction first appears in a newly-attached block.|
|Returns|Nothing|
[Return to Overview](#WSExtMethodOverview)<br />
...
abcd uses standard JSON-RPC notifications to notify clients of changes, rather than requiring clients to poll abcd for updates.  JSON-RPC notifications are a subset of requests, but do not contain an ID.  The notification type is categorized by the `method` field and additional details are sent as a JSON array in the `params` field.
...
[abcrpcclient](https://github.com/abcsuite/abcrpcclient) package.
...
[abcrpcclient](https://github.com/abcsuite/abcrpcclient) package to connect with
a abcd instance via Websockets, issues [getblockcount](#getblockcount) to
...
func main() {
	...
	// generated by abcd when it starts the RPC server and doesn't already
	// have one.
...
The following is an example Go application which uses the
[abcrpcclient](https://github.com/abcsuite/abcrpcclient) package to connect with
a abcd instance via Websockets, issues [getblock](#getblock) to retrieve
...
The following is an example Go application which uses the
[abcrpcclient](https://github.com/abcsuite/abcrpcclient) package to connect with
a abcd instance via Websockets and registers for
...
(can be installed with `npm install ws`) to connect with a abcd instance,
...
// generated by abcd when it starts the RPC server and doesn't already
// have one.
var cert = fs.readFileSync('/path/to/abcd/appdata/rpc.cert');
var user = "yourusername";
var password = "yourpassword";


// Initiate the websocket connection.  The abcd generated certificate acts as
// its own certificate authority, so it needs to be specified in the 'ca' array
// for the certificate to properly validate.
var ws = new WebSocket('wss://127.0.0.1:9528/ws', {
  headers: {
...
As mentioned in the [overview](#Overview), the websocket connection endpoint for
btcd is `wss://your_ip_or_domain:9526/ws`.
...
// its own certificate authority, so it needs to be specified in the 'ca' array
// for the certificate to properly validate.
var ws = new WebSocket('wss://127.0.0.1:9526/ws', {
  headers: {
    'Authorization': 'Basic '+new Buffer(user+':'+password).toString('base64')
  },
  cert: cert,
