...
abcd provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, abcd supports a hybrid,
...
While it is easier to only run as a client, it is more beneficial to the Aero
...
Configuring abcd as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the abcd command
line or in the abcd configuration file.  Typically the Tor proxy address will be
...
By default, abcd assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since abcd will act as the server.
...
```text
HiddenServiceDir /var/tor/abcd
HiddenServicePort 9527 127.0.0.1:9527
```

Once Tor is configured to provide the hidden service and you have obtained your
generated .onion address, configuring abcd as a Tor hidden service requires
three flags:
...
```bash
$ ./abcd --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
abcd provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the abcd command line or in the abcd
...
```bash
$ ./abcd --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
abcd provides support for Tor stream isolation by using the `--torisolation`
...
```bash
$ ./abcd --proxy=127.0.0.1:9050 --torisolation
```
