...
import (
	...
	"github.com/abcsuite/abcd/limits"
)

var cfg *config

// winServiceMain is only invoked on Windows.  It detects when abcd is running
// as a service and reacts accordingly.
var winServiceMain func() (bool, error)

// abcdMain is the real main function for abcd.  It is necessary to work around
// the fact that deferred functions do not run when os.Exit() is called.  The
// optional serverChan parameter is mainly used by the service code to be
// notified with the server once it is setup so it can gracefully stop it when
// requested from the service control manager.
func abcdMain(serverChan chan<- *server) error {
	...
	defer abcdLog.Info("Shutdown complete")

	// Show version and home dir at startup.
	abcdLog.Infof("Version %s (Go version %s)", version(), runtime.Version())
	abcdLog.Infof("Home dir: %s", cfg.HomeDir)

	// Enable http profiling server if requested.
	if cfg.Profile != "" {
		go func() {
			listenAddr := net.JoinHostPort("", cfg.Profile)
			abcdLog.Infof("Creating profiling server "+
	...
	if cfg.CPUProfile != "" {
		...
		if err != nil {
			abcdLog.Errorf("Unable to create cpu profile: %v", err.Error())
			return err
		}
	...
	if cfg.MemProfile != "" {
		...
		if err != nil {
			abcdLog.Errorf("Unable to create mem profile: %v", err)
			return err
		}
	...
	db, err := loadBlockDB()
	if err != nil {
		abcdLog.Errorf("%v", err)
		return err
	}
	defer func() {
		// Ensure the database is sync'd and closed on shutdown.
		lifetimeNotifier.notifyShutdownEvent(lifetimeEventDBOpen)
		abcdLog.Infof("Gracefully shutting down the database...")
		db.Close()
	}()
	...
	if err != nil {
		...
		abcdLog.Errorf("Unable to start server on %v: %v",
			cfg.Listeners, err)
		return err
	}
	defer func() {
		lifetimeNotifier.notifyShutdownEvent(lifetimeEventP2PServer)
		abcdLog.Infof("Gracefully shutting down the server...")
		server.Stop()
