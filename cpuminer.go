...
func (m *CPUMiner) submitBlock(block *abcutil.Block) bool {
	...
	minrLog.Infof("Block submitted via CPU miner accepted (hash %s, "+
		...
		abcutil.Amount(coinbaseTxGenerated))
	return true
}
...
out:
	for {
		...
		// Hacks to make abc work with Aero PoC (simnet only)
		...
		if m.solveBlock(template.Block, ticker, quit) {
			block := abcutil.NewBlock(template.Block)
			m.submitBlock(block)
