...
const (
	defaultConfigFilename        = "abcd.conf"
	defaultDataDirname           = "data"
	defaultLogLevel              = "info"
	defaultLogDirname            = "logs"
	defaultLogFilename           = "abcd.log"
	defaultMaxPeers              = 125
	...
	sampleConfigFilename         = "sample-abcd.conf"
	defaultTxIndex               = false
	defaultNoExistsAddrIndex     = false
)

var (
	defaultHomeDir     = abcutil.AppDataDir("abcd", false)
...
// config defines the configuration options for abcd.
//
...
type config struct {
	...
	Listeners            []string      `long:"listen" description:"Add an interface/port to listen for connections (default all interfaces port: 9527, testnet: 19527)"`
	...
	RPCListeners         []string      `long:"rpclisten" description:"Add an interface/port to listen for RPC connections (default port: 9528, testnet: 19529)"`
	...
	MinRelayTxFee        float64       `long:"minrelaytxfee" description:"The minimum transaction fee in ABC/kB to be considered a non-zero fee."`
...
//
// The above results in abcd functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	// Update the home directory for abcd if specified. Since the home
	...
	cfg.minRelayTxFee, err = abcutil.NewAmount(cfg.MinRelayTxFee)
	if err != nil {
	...
	cfg.miningAddrs = make([]abcutil.Address, 0, len(cfg.GetWorkKeys)+
		len(cfg.MiningAddrs))
	for _, strAddr := range cfg.GetWorkKeys {
		addr, err := abcutil.DecodeAddress(strAddr,
	...
	for _, strAddr := range cfg.MiningAddrs {
		addr, err := abcutil.DecodeAddress(strAddr, activeNetParams.Params)
		if err != nil {
	...
	for _, oldDir := range oldTestNets {
		if fileExists(oldDir) {
			abcdLog.Warnf("Block chain data from previous testnet"+
	...
	if configFileError != nil {
		abcdLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// createDefaultConfig copies the file sample-abcd.conf to the given destination path,
...
// abcdDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func abcdDial(addr net.Addr) (net.Conn, error) {
...
// abcdLookup returns the correct DNS lookup function to use depending on the
...
func abcdLookup(host string) ([]net.IP, error) {
