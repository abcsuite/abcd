...
// MessageHeaderSize is the number of bytes in a abcd message header.
// Aero network (magic) 4 bytes + command 12 bytes + payload length 4 bytes +
// checksum 4 bytes.
const MessageHeaderSize = 24

// CommandSize is the fixed size of all commands in the common abcd message
...
// Message is an interface that describes a abcd message.  A type that
...
// messageHeader defines the header structure for all abcd protocol messages.
...
// readMessageHeader reads a abcd message header from r.
...
// WriteMessage writes a abcd Message to w including the necessary header
...
// ReadMessageN reads, validates, and parses the next abcd Message from r for
// the provided protocol version and abcd network.  It returns the number of
...
// ReadMessage reads, validates, and parses the next abcd Message from r for
// the provided protocol version and abcd network.  It returns the parsed
