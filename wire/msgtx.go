...
// this msgTx is. You can use a bitmask for this too, but Aero just splits
...
// OutPoint defines a abcd data type that is used to track previous
// transaction outputs.
...
// NewOutPoint returns a new abcd transaction outpoint point with the
...
// TxIn defines a abcd transaction input.
type TxIn struct {
	// Non-witness
...
// NewTxIn returns a new abcd transaction input with the provided
...
// TxOut defines a abcd transaction output.
type TxOut struct {
	Value    int64
	Version  uint16
	PkScript []byte
...
// NewTxOut returns a new abcd transaction output with the provided
...
// MsgTx implements the Message interface and represents a abcd tx message.
...
func writeTxScriptsToMsgTx(msg *MsgTx, totalScriptSize uint64, mType TxSerializeType) {
	...
	//
	// Closures around writing the TxIn and TxOut scripts are used in Aero
...
// BtcDecode decodes r using the abcd protocol encoding into the receiver.
...
// LegacyBtcDecode decodes r using the abcd protocol encoding into the
...
// in that BtcDecode decodes from the Aero wire protocol as it was sent
...
// BtcEncode encodes the receiver to w using the Aero protocol encoding.
// This is part of the Message interface implementation.
...
// LegacyBtcEncode encodes the receiver to w using the Aero protocol encoding.
...
// encodes the transaction to the abcd wire protocol in order to be sent
// across the network.  The wire encoding can technically differ depending on
// the protocol version and doesn't even really need to match the format of a
// stored transaction at all.  As of the time this comment was written, the
// encoded transaction is the same in both instances, but there is a distinct
...
// LegacySerialize encodes the transaction in abcd legacy format, for
// compatibility.
...
// appropriate transaction output entry. This is for legacy abcd format.
...
// NewMsgTx returns a new abcd tx message that conforms to the Message
...
// WriteOutPoint encodes op to the abcd protocol encoding for an OutPoint
// to w.
...
// legacyWriteOutPoint encodes op to the abcd protocol encoding for a legacy
// Aero OutPoint to w.
...
// legacyWriteTxIn encodes ti to the abcd protocol encoding for a transaction
// input (TxIn) to w for abcd legacy format.
...
// writeTxInPrefixs encodes ti to the abcd protocol encoding for a transaction
...
// writeTxWitness encodes ti to the abcd protocol encoding for a transaction
...
// writeTxInWitnessSigning encodes ti to the abcd protocol encoding for a
...
// writeTxInWitnessValueSigning encodes ti to the abcd protocol encoding for a
...
// writeTxOut encodes to into the abcd protocol encoding for a transaction
// output (TxOut) to w.
...
// (TxOut) in legacy Aero format (for tests).
...
// legacyWriteTxOut encodes to into the abcd protocol encoding for a transaction
// output (TxOut) to w in legacy Aero format (for tests).
