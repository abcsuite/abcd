...
var (
	...
	// data.
	gbtCoinbaseAux = &abcjson.GetBlockTemplateResultAux{
...
var (
	...
	ErrRPCUnimplemented = &abcjson.RPCError{
	...
	ErrRPCNoWallet = &abcjson.RPCError{
		Code:    abcjson.ErrRPCNoWallet,
...
// list of commands that we recognize, but for which abcd has no support because
// it lacks support for wallet functionality. For these commands the user
// should ask a connected instance of Abcwallet.
...
// not needed.
func rpcInternalError(errStr, context string) *abcjson.RPCError {
	logStr := errStr
	if context != "" {
		logStr = context + ": " + errStr
	}
	rpcsLog.Error(logStr)
	return abcjson.NewRPCError(abcjson.ErrRPCInternal.Code, errStr)
}
...
func rpcInvalidError(fmtStr string, args ...interface{}) *abcjson.RPCError {
	return abcjson.NewRPCError(abcjson.ErrRPCInvalidParameter,
...
func rpcDeserializationError(fmtStr string, args ...interface{}) *abcjson.RPCError {
	return abcjson.NewRPCError(abcjson.ErrRPCDeserialization,
		fmt.Sprintf(fmtStr, args...))
}
...
// context parameter is only used in the log message and may be empty if it's
// not needed.
func rpcAddressKeyError(fmtStr string, args ...interface{}) *abcjson.RPCError {
	return abcjson.NewRPCError(abcjson.ErrRPCInvalidAddressOrKey,
		fmt.Sprintf(fmtStr, args...))
}

// rpcDecodeHexError is a convenience function for returning a nicely formatted
// RPC error which indicates the provided hex string failed to decode.
func rpcDecodeHexError(gotHex string) *abcjson.RPCError {
	return abcjson.NewRPCError(abcjson.ErrRPCDecodeHexString,
...
// transaction hash.
func rpcNoTxInfoError(txHash *chainhash.Hash) *abcjson.RPCError {
	return abcjson.NewRPCError(abcjson.ErrRPCNoTxInfo,
...
func rpcMiscError(message string) *abcjson.RPCError {
	return abcjson.NewRPCError(abcjson.ErrRPCMisc, message)
}
...
// These commands will be implemented in Abcwallet.
...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abcjson.CreateRawTransactionCmd)
	...
	for encodedAddr, amount := range c.Amounts {
		...
		if amount <= 0 || amount > abcutil.MaxAmount {
			return nil, rpcInvalidError("Invalid amount: 0 >= %v "+
				"> %v", amount, abcutil.MaxAmount)
		}

		// Decode the provided address.
		addr, err := abcutil.DecodeAddress(encodedAddr,
		...
		atomic, err := abcutil.NewAmount(amount)
		if err != nil {
			return nil, rpcInternalError(err.Error(),
				"New amount")
		}
...
func handleCreateRawSStx(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abcjson.CreateRawSStxCmd)
	...
	for i, cout := range c.COuts {
		...
		addr, err := abcutil.DecodeAddress(cout.Addr,
			activeNetParams.Params)
		if err != nil {
			return nil, rpcAddressKeyError("Could not decode "+
				"address: %v", err)
		...
		pkScript, err := txscript.GenerateSStxAddrPush(addr,
			abcutil.Amount(amountsCommitted[i]), 0x0000)
		if err != nil {
			return nil, rpcInternalError(err.Error(),
		...
		// Ensure amount is in the valid range for monetary amounts.
		if cout.ChangeAmt < 0 || cout.ChangeAmt > abcutil.MaxAmount {
			return nil, rpcInvalidError("Invalid change amount: 0 "+
				"> %v > %v", cout.ChangeAmt, abcutil.MaxAmount)
		}

		// Decode the provided address.
		addr, err = abcutil.DecodeAddress(cout.ChangeAddr,
			activeNetParams.Params)
		if err != nil {
			return nil, rpcAddressKeyError("Wrong network: %v",
				addr)
...
func handleCreateRawSSGenTx(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abcjson.CreateRawSSGenTxCmd)
	...
	for i, ssgenPkh := range ssgenPkhs {
		...
		if ssgenCalcAmts[i] <= 0 ||
			ssgenCalcAmts[i] > abcutil.MaxAmount {
...
func handleCreateRawSSRtx(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abcjson.CreateRawSSRtxCmd)
	...
	var feeAmt abcutil.Amount
	if c.Fee != nil {
		var err error
		feeAmt, err = abcutil.NewAmount(*c.Fee)
		if err != nil {
	...
	for i, ssrtxPkh := range ssrtxPkhs {
		// Ensure amount is in the valid range for monetary amounts.
		if sstxAmts[i] <= 0 || sstxAmts[i] > abcutil.MaxAmount {
			return nil, rpcInvalidError("Invalid SSTx amount: 0 >="+
				" %v > %v", sstxAmts[i] <= 0, abcutil.MaxAmount)
		}
...
// transaction.
func createVinList(mtx *wire.MsgTx) []abcjson.Vin {
	// Coinbase transactions only have a single txin by definition.
	vinList := make([]abcjson.Vin, len(mtx.TxIn))
	if blockchain.IsCoinBaseTx(mtx) {
		txIn := mtx.TxIn[0]
		vinEntry := &vinList[0]
		vinEntry.Coinbase = hex.EncodeToString(txIn.SignatureScript)
		vinEntry.Sequence = txIn.Sequence
		vinEntry.AmountIn = abcutil.Amount(txIn.ValueIn).ToCoin()
	...
	for i, txIn := range mtx.TxIn {
		...
		vinEntry.BlockHeight = txIn.BlockHeight
		vinEntry.BlockIndex = txIn.BlockIndex
		vinEntry.ScriptSig = &abcjson.ScriptSig{
			Asm: disbuf,
...
// transaction.
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []abcjson.Vout {

	txType := stake.DetermineTxType(mtx)
	voutList := make([]abcjson.Vout, 0, len(mtx.TxOut))
	for i, v := range mtx.TxOut {
		...
		var vout abcjson.Vout
		voutSPK := &vout.ScriptPubKey
		vout.N = uint32(i)
		vout.Value = abcutil.Amount(v.Value).ToCoin()
		...
		if commitAmt != nil {
			voutSPK.CommitAmt = abcjson.Float64(commitAmt.ToCoin())
		}
...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx, txHash string, blkIdx uint32, blkHeader *wire.BlockHeader, blkHash string, blkHeight int64, confirmations int64) (*abcjson.TxRawResult, error) {
	mtxHex, err := messageToHex(mtx)
	if err != nil {
		return nil, err
	}
...
func handleDecodeRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abcjson.DecodeRawTransactionCmd)
	...
	txReply := abcjson.TxRawDecodeResult{
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	p2sh, err := abcutil.NewAddressScriptHash(script, s.server.chainParams)
	if err != nil {
		return nil, rpcInternalError(err.Error(),
	...
	reply := abcjson.DecodeScriptResult{
		Asm:       disbuf,
...
func handleEstimateStakeDiff(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abcjson.EstimateStakeDiffCmd)
	...
	if c.Tickets != nil {
		...
		userEstFlt := abcutil.Amount(userEst).ToCoin()
		userEstFltPtr = &userEstFlt
	}

	return &abcjson.EstimateStakeDiffResult{
		Min:      abcutil.Amount(min).ToCoin(),
		Max:      abcutil.Amount(max).ToCoin(),
		Expected: abcutil.Amount(expected).ToCoin(),
		User:     userEstFltPtr,
	}, nil
}
...
func handleExistsAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*abcjson.ExistsAddressCmd)

	// Attempt to decode the supplied address.
	addr, err := abcutil.DecodeAddress(c.Address, s.server.chainParams)
	if err != nil {
...
func handleExistsAddresses(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*abcjson.ExistsAddressesCmd)
	addresses := make([]abcutil.Address, len(c.Addresses))
	for i := range c.Addresses {
		// Attempt to decode the supplied address.
		addr, err := abcutil.DecodeAddress(c.Addresses[i], s.server.chainParams)
		if err != nil {
			return nil, rpcAddressKeyError("Could not decode "+
				"address: %v", err)
		}
...
func handleExistsMissedTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abcjson.ExistsMissedTicketsCmd)

	hashes, err := abcjson.DecodeConcatenatedHashes(c.TxHashBlob)
	if err != nil {
		return nil, err
	}
...
func handleExistsExpiredTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abcjson.ExistsExpiredTicketsCmd)
...
func handleExistsLiveTicket(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abcjson.ExistsLiveTicketCmd)
...
func handleExistsLiveTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abcjson.ExistsLiveTicketsCmd)
...
func handleExistsMempoolTxs(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abcjson.ExistsMempoolTxsCmd)
...
func handleGetAddedNodeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abcjson.GetAddedNodeInfoCmd)

	// Retrieve a list of persistent (added) peers from the abcd server
	...
	results := make([]*abcjson.GetAddedNodeInfoResult, 0, len(peers))
	for _, peer := range peers {
		// Set the "address" of the peer which could be an ip address
		// or a domain name.
		var result abcjson.GetAddedNodeInfoResult
		result.AddedNode = peer.Addr()
		result.Connected = abcjson.Bool(peer.Connected())
		...
		addrs := make([]abcjson.GetAddedNodeInfoResultAddr, 0,
			len(ipList))
		for _, ip := range ipList {
			var addr abcjson.GetAddedNodeInfoResultAddr
			addr.Address = ip
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	sbitsFloat := float64(blockHeader.SBits) / abcutil.AtomsPerCoin
	blockReply := abcjson.GetBlockVerboseResult{
		Hash:          c.Hash,
	...
	if c.VerboseTx == nil || !*c.VerboseTx {
		...
		rawTxns := make([]abcjson.TxRawResult, len(txns))
		for i, tx := range txns {
		...
		rawSTxns := make([]abcjson.TxRawResult, len(stxns))
...
func handleGetBlockHash(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abcjson.GetBlockHashCmd)
	hash, err := s.chain.BlockHashByHeight(c.Index)
	if err != nil {
		return nil, &abcjson.RPCError{
			Code: abcjson.ErrRPCOutOfRange,
...
func handleGetBlockHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abcjson.GetBlockHeaderCmd)
	...
	blockHeaderReply := abcjson.GetBlockHeaderVerboseResult{
		Hash:          c.Hash,
		Confirmations: confirmations,
		Version:       blockHeader.Version,
		PreviousHash:  blockHeader.PrevBlock.String(),
		...
		PoolSize:      blockHeader.PoolSize,
		Bits:          strconv.FormatInt(int64(blockHeader.Bits), 16),
		SBits:         abcutil.Amount(blockHeader.SBits).ToCoin(),
...
func handleGetBlockSubsidy(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abcjson.GetBlockSubsidyCmd)
	...
	rep := abcjson.GetBlockSubsidyResult{
		Developer: dev,
		PoS:       pos,
		PoW:       pow,
		Total:     total,
...
// state as a abcjson.GetBlockTemplateResult that is ready to be encoded to
// JSON and returned to the caller.
//
// This function MUST be called with the state locked.
func (state *gbtWorkState) blockTemplateResult(bm *blockManager, useCoinbaseValue bool, submitOld *bool) (*abcjson.GetBlockTemplateResult, error) {
	...
	if newestBlock == nil {
		return nil, &abcjson.RPCError{
			Code:    abcjson.ErrRPCBestBlockHash,
	...
	transactions := make([]abcjson.GetBlockTemplateResultTx, 0, numTx-1)
	...
	for i, tx := range msgBlock.Transactions {
		...
		if !recalculateFeesAndSigsOps {
			...
			txU := abcutil.NewTx(tx)
			isValid := abcutil.IsFlagSet16(
		...
		resultTx := abcjson.GetBlockTemplateResultTx{
	...
	stransactions := make([]abcjson.GetBlockTemplateResultTx, 0, numSTx)
	...
	reply := abcjson.GetBlockTemplateResult{
...
func handleGetBlockTemplateRequest(s *rpcServer, request *abcjson.TemplateRequest, closeChan <-chan struct{}) (interface{}, error) {
	...
	if !cfg.SimNet && s.server.ConnectedCount() == 0 {
		return nil, &abcjson.RPCError{
			Code:    abcjson.ErrRPCClientNotConnected,
			Message: "Aero is not connected",
		}
	}
	...
	if currentHeight != 0 && !s.server.blockManager.IsCurrent() {
		return nil, &abcjson.RPCError{
			Code:    abcjson.ErrRPCClientInInitialDownload,
			Message: "Aero is downloading blocks...",
		}
	}
...
// reasons.
// TODO Aero pop in the new errors from blockchain cj
...
func handleGetBlockTemplateProposal(s *rpcServer, request *abcjson.TemplateRequest) (interface{}, error) {
...
func handleGetBlockTemplate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*abcjson.GetBlockTemplateCmd)
	request := c.Request
...
func handleGetHeaders(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abcjson.GetHeadersCmd)
	blockLocators, err := abcjson.DecodeConcatenatedHashes(c.BlockLocators)
	if err != nil {
		// Already a *abcjson.RPCError
		return nil, err
	}
	...
	return &abcjson.GetHeadersResult{Headers: hexBlockHeaders}, nil
}
...
func handleGetMempoolInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &abcjson.GetMempoolInfoResult{
...
func handleGetMiningInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	gnhpsCmd := abcjson.NewGetNetworkHashPSCmd(nil, nil)
	...
	result := abcjson.GetMiningInfoResult{
		Blocks:           best.Height,
...
func handleGetNetTotals(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	reply := &abcjson.GetNetTotalsResult{
...
func handleGetNetworkHashPS(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*abcjson.GetNetworkHashPSCmd)
...
func handleGetPeerInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	infos := make([]*abcjson.GetPeerInfoResult, 0, len(peers))
	for _, p := range peers {
		statsSnap := p.StatsSnapshot()
		info := &abcjson.GetPeerInfoResult{
			ID:             statsSnap.ID,
			Addr:           statsSnap.Addr,
...
func handleGetRawMempool(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abcjson.GetRawMempoolCmd)
	...
	if c.TxType != nil {
		switch abcjson.GetRawMempoolTxTypeCmd(*c.TxType) {
		case abcjson.GRMRegular:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeRegular
		case abcjson.GRMTickets:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeSStx
		case abcjson.GRMVotes:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeSSGen
		case abcjson.GRMRevocations:
			filterType = new(stake.TxType)
...
func handleGetRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abcjson.GetRawTransactionCmd)
...
func handleGetStakeDifficulty(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if err != nil {
		...
		return nil, &abcjson.RPCError{
			Code:    abcjson.ErrRPCDifficulty,
			Message: "Error getting stake difficulty: " + err.Error(),
		}
	}
	currentSdiff := abcutil.Amount(blockHeader.SBits)
	...
	nextSdiffAmount := abcutil.Amount(nextSdiff)

	sDiffResult := &abcjson.GetStakeDifficultyResult{
...
func convertVersionMap(m map[int]int) []abcjson.VersionCount {
	sorted := make([]abcjson.VersionCount, 0, len(m))
	...
	for _, v := range order {
		sorted = append(sorted, abcjson.VersionCount{Version: uint32(v),
			Count: uint32(m[v])})
	}
...
func handleGetStakeVersionInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	count := int32(1)
	c, ok := cmd.(*abcjson.GetStakeVersionInfoCmd)
	if !ok {
	...
	result := abcjson.GetStakeVersionInfoResult{
		CurrentHeight: snapshot.Height,
		Hash:          snapshot.Hash.String(),
		Intervals:     make([]abcjson.VersionInterval, 0, count),
	}
	...
	for i := int32(0); i < count; i++ {
		...
		versionInterval := abcjson.VersionInterval{
			StartHeight:  endHeight,
...
func handleGetStakeVersions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abcjson.GetStakeVersionsCmd)
	...
	result := abcjson.GetStakeVersionsResult{
		StakeVersions: make([]abcjson.StakeVersions, 0, len(sv)),
	}
	for _, v := range sv {
		nsv := abcjson.StakeVersions{
			Hash:         v.Hash.String(),
			Height:       v.Height,
			BlockVersion: v.BlockVersion,
			StakeVersion: v.StakeVersion,
			Votes: make([]abcjson.VersionBits, 0,
				len(v.Votes)),
		}
		for _, vote := range v.Votes {
			nsv.Votes = append(nsv.Votes,
				abcjson.VersionBits{Version: vote.Version,
					Bits: vote.Bits})
		}
...
func handleGetVoteInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c, ok := cmd.(*abcjson.GetVoteInfoCmd)
	if !ok {
		return nil, rpcInvalidError("Invalid type: %T", c)
	}
	...
	// Assemble JSON result.
	result := abcjson.GetVoteInfoResult{
		CurrentHeight: snapshot.Height,
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if txFromMempool != nil {
		...
		if c.Vout > uint32(len(mtx.TxOut)-1) {
			return nil, &abcjson.RPCError{
				Code: abcjson.ErrRPCInvalidTxVout,
	...
	txOutReply := &abcjson.GetTxOutResult{
		BestBlock:     bestBlockHash,
		Confirmations: confirmations,
		Value:         abcutil.Amount(value).ToUnit(abcutil.AmountCoin),
		Version:       txVersion,
		ScriptPubKey: abcjson.ScriptPubKeyResult{
			Asm:       disbuf,
...
func handleGetWorkSubmission(s *rpcServer, hexData string) (interface{}, error) {
	...
	// blockchain.
	tempBlock := abcutil.NewBlockDeepCopy(blockInfo.msgBlock)
...
func handleLiveTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	return abcjson.LiveTicketsResult{Tickets: ltString}, nil
}
...
func handleMissedTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	return abcjson.MissedTicketsResult{Tickets: mtString}, nil
}
...
// passed transaction.
func createVinListPrevOut(s *rpcServer, mtx *wire.MsgTx, chainParams *chaincfg.Params, vinExtra bool, filterAddrMap map[string]struct{}) ([]abcjson.VinPrevOut, error) {
	// Coinbase transactions only have a single txin by definition.
	if blockchain.IsCoinBaseTx(mtx) {
		...
		txIn := mtx.TxIn[0]
		vinList := make([]abcjson.VinPrevOut, 1)
		vinList[0].Coinbase = hex.EncodeToString(txIn.SignatureScript)
		amountIn := abcutil.Amount(txIn.ValueIn).ToCoin()
	...
	vinList := make([]abcjson.VinPrevOut, 0, len(mtx.TxIn))
	...
	for _, txIn := range mtx.TxIn {
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &abcjson.PrevOut{
				Addresses: encodedAddrs,
				Value:     abcutil.Amount(originTxOut.Value).ToCoin(),
			}
		}
	}
...
func fetchMempoolTxnsForAddress(s *rpcServer, addr abcutil.Address, numToSkip, numRequested uint32) ([]*abcutil.Tx, uint32) {
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*abcjson.SearchRawTransactionsCmd)
	vinExtra := false
	if c.VinExtra != nil {
	...
	srtList := make([]abcjson.SearchRawTransactionsResult, len(addressTxns))
	for i := range addressTxns {
...
func handleSendRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abcjson.SendRawTransactionCmd)
...
func min(s []abcutil.Amount) abcutil.Amount {
	if len(s) == 0 {
		return 0
	}
...
func max(s []abcutil.Amount) abcutil.Amount {
	max := abcutil.Amount(0)
	for i := range s {
		if s[i] > max {
			max = s[i]
		}
...
func mean(s []abcutil.Amount) abcutil.Amount {
	sum := abcutil.Amount(0)
	for i := range s {
		sum += s[i]
	}
	...
	return sum / abcutil.Amount(len(s))
}

// median gets the median amount from a slice of amounts.
func median(s []abcutil.Amount) abcutil.Amount {
	if len(s) == 0 {
		return 0
	}

	sort.Sort(abcutil.AmountSorter(s))
...
func stdDev(s []abcutil.Amount) abcutil.Amount {
	var total float64
	mean := mean(s)
	...
	amt, _ := abcutil.NewAmount(math.Sqrt(v))
...
// memory pool.
func feeInfoForMempool(s *rpcServer, txType stake.TxType) *abcjson.FeeInfoMempool {
	txDs := s.server.txMemPool.TxDescs()
	ticketFees := make([]abcutil.Amount, 0, len(txDs))
	for _, txD := range txDs {
		if txD.Type == txType {
			feePerKb := (abcutil.Amount(txD.Fee)) * 1000 /
				abcutil.Amount(txD.Tx.MsgTx().SerializeSize())
			ticketFees = append(ticketFees, feePerKb)
		}
	}

	return &abcjson.FeeInfoMempool{
...
// properly set.
func calcFeePerKb(tx *abcutil.Tx) abcutil.Amount {
	var in abcutil.Amount
	for _, txIn := range tx.MsgTx().TxIn {
		in += abcutil.Amount(txIn.ValueIn)
	}
	var out abcutil.Amount
	for _, txOut := range tx.MsgTx().TxOut {
		out += abcutil.Amount(txOut.Value)
	}

	return ((in - out) * 1000) / abcutil.Amount(tx.MsgTx().SerializeSize())
}

// feeInfoForBlock fetches the ticket fee information for a given tx type in a
// block.
func ticketFeeInfoForBlock(s *rpcServer, height int64, txType stake.TxType) (*abcjson.FeeInfoBlock, error) {
	...
	txFees := make([]abcutil.Amount, txNum)
	itr := 0
...
// from [start, end).
func ticketFeeInfoForRange(s *rpcServer, start int64, end int64, txType stake.TxType) (*abcjson.FeeInfoWindow, error) {
	hashes, err := s.chain.HeightRange(start, end)
	if err != nil {
		return nil, err
	}

	var txFees []abcutil.Amount
	for i := range hashes {
...
func handleTicketFeeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abcjson.TicketFeeInfoCmd)
	...
	var feeInfoBlocks []abcjson.FeeInfoBlock
	blocks := uint32(0)
	if c.Blocks != nil {
		blocks = *c.Blocks
	}
	...
	var feeInfoWindows []abcjson.FeeInfoWindow
	windows := uint32(0)
	if c.Windows != nil {
		windows = *c.Windows
	}
	...
	return &abcjson.TicketFeeInfoResult{
...
func handleTicketsForAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abcjson.TicketsForAddressCmd)
	...
	reply := &abcjson.TicketsForAddressResult{
...
func handleTicketVWAP(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	return abcutil.Amount(vwap).ToCoin(), nil
}
...
func handleTxFeeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	var feeInfoRange abcjson.FeeInfoRange
	...
	feeInfoRange = abcjson.FeeInfoRange{
	...
	return &abcjson.TxFeeInfoResult{
		FeeInfoMempool: *feeInfoMempool,
		FeeInfoBlocks:  feeInfoBlocks,
...
func handleValidateAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abcjson.ValidateAddressCmd)

	result := abcjson.ValidateAddressChainResult{}
	addr, err := abcutil.DecodeAddress(c.Address, activeNetParams.Params)
	if err != nil {
...
func handleVerifyMessage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*abcjson.VerifyMessageCmd)
	...
	if _, ok := addr.(*abcutil.AddressPubKeyHash); !ok {
		return nil, &abcjson.RPCError{
			Code:    abcjson.ErrRPCType,
...
func handleVersion(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	result := map[string]abcjson.VersionResult{
		"abcdjsonrpcapi": {
...
func (s *rpcServer) standardCmdResult(cmd *parsedRPCCmd, closeChan <-chan struct{}) (interface{}, error) {
	...
	return nil, abcjson.ErrRPCMethodNotFound
handled:
...
func parseCmd(request *abcjson.Request) *parsedRPCCmd {
	...
	if err != nil {
		...
		if jerr, ok := err.(abcjson.Error); ok &&
			jerr.Code == abcjson.ErrUnregisteredMethod {

			parsedCmd.err = abcjson.ErrRPCMethodNotFound
			return &parsedCmd
		}
...
// the type *abcjson.RPCError to the appropriate type as needed.
func createMarshalledReply(id, result interface{}, replyErr error) ([]byte, error) {
	var jsonErr *abcjson.RPCError
	if replyErr != nil {
		if jErr, ok := replyErr.(*abcjson.RPCError); ok {
			jsonErr = jErr
		} else {
	...
	return abcjson.MarshalResponse(id, result, jsonErr)
}
...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="abcd RPC"`)
...
func genCertPair(certFile, keyFile string) error {
	...
	org := "abcd autogenerated cert"
	validUntil := time.Now().Add(10 * 365 * 24 * time.Hour)
	cert, key, err := abcutil.NewTLSCertPair(elliptic.P521(), org,
		validUntil, nil)
	if err != nil {
		return err
	}
