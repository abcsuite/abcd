...
func serviceControlPipeRx(fd uintptr) {
	...
	for {
		...
		if err != nil {
			abcdLog.Errorf("Failed to read from pipe: %v", err)
			break
		}
	}
...
// for a child abcd process to communicate with a parent process without the
...
func serviceControlPipeTx(fd uintptr) {
	...
	abcdLog.Errorf("Failed to write to pipe: %v", err)
}
