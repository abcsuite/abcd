...
import (
	...
	"github.com/abcsuite/abcd/mining"
...
const (
	...
	MinHighPriority = abcutil.AtomsPerCoin * 144.0 / 250
	...
	// ABC/kB, this results in a maximum allowed high fee of 1 ABC/kB.
...
type Config struct {
	...
	FetchUtxoView func(*abcutil.Tx, bool) (*blockchain.UtxoViewpoint, error)
...
type TxPool struct {
	...
	orphans       map[chainhash.Hash]*abcutil.Tx
	orphansByPrev map[chainhash.Hash]map[chainhash.Hash]*abcutil.Tx
	addrindex     map[string]map[chainhash.Hash]struct{} // maps address to txs
	outpoints     map[wire.OutPoint]*abcutil.Tx
...
func (mp *TxPool) insertVote(ssgen *abcutil.Tx) error {
	voteHash := ssgen.Hash()
	msgTx := ssgen.MsgTx()
	...
	vote := abcutil.IsFlagSet16(voteBits, abcutil.BlockValid)
...
// TODO Pruning of the votes map Aero
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addOrphan(tx *abcutil.Tx) {
	...
	for _, txIn := range tx.MsgTx().TxIn {
		...
		if _, exists := mp.orphansByPrev[originTxHash]; !exists {
			mp.orphansByPrev[originTxHash] =
				make(map[chainhash.Hash]*abcutil.Tx)
		}
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAddOrphan(tx *abcutil.Tx) error {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeTransaction(tx *abcutil.Tx, removeRedeemers bool) {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RemoveDoubleSpends(tx *abcutil.Tx) {
	// Protect concurrent access.
	mp.Lock()
	defer mp.Unlock()
...
func (mp *TxPool) addTransaction(utxoView *blockchain.UtxoViewpoint,
	tx *abcutil.Tx, txType stake.TxType, height int64, fee int64) {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) checkPoolDoubleSpend(tx *abcutil.Tx, txType stake.TxType) error {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) fetchInputUtxos(tx *abcutil.Tx) (*blockchain.UtxoViewpoint, error) {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) FetchTransaction(txHash *chainhash.Hash, includeRecentBlock bool) (*abcutil.Tx, error) {
	// Protect concurrent access.
	mp.RLock()
	defer mp.RUnlock()
	...
	// For Aero, the latest block is considered "unconfirmed"
...
// It should also set the abcutil tree type for the tx as well.
func (mp *TxPool) maybeAcceptTransaction(tx *abcutil.Tx, isNew, rateLimit, allowHighFees bool) ([]*chainhash.Hash, error) {
	msgTx := tx.MsgTx()
	txHash := tx.Hash()
...
//
// This function is safe for concurrent access.
func (mp *TxPool) MaybeAcceptTransaction(tx *abcutil.Tx, isNew, rateLimit bool) ([]*chainhash.Hash, error) {
	// Protect concurrent access.
	mp.Lock()
	defer mp.Unlock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) processOrphans(hash *chainhash.Hash) []*abcutil.Tx {
	var acceptedTxns []*abcutil.Tx
...
//
// This function is safe for concurrent access.
func (mp *TxPool) ProcessTransaction(tx *abcutil.Tx, allowOrphan, rateLimit, allowHighFees bool) ([]*abcutil.Tx, error) {
	// Protect concurrent access.
	mp.Lock()
	defer mp.Unlock()
	var err error
	...
	if len(missingParents) == 0 {
		...
		acceptedTxs := make([]*abcutil.Tx, len(newTxs)+1)
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RawMempoolVerbose(filterType *stake.TxType) map[string]*abcjson.GetRawMempoolVerboseResult {
	mp.RLock()
	defer mp.RUnlock()

	result := make(map[string]*abcjson.GetRawMempoolVerboseResult,
		len(mp.pool))
	best := mp.cfg.Chain.BestSnapshot()
	...
	for _, desc := range mp.pool {
		...
		mpd := &abcjson.GetRawMempoolVerboseResult{
			Size:             int32(tx.MsgTx().SerializeSize()),
			Fee:              abcutil.Amount(desc.Fee).ToCoin(),
...
func New(cfg *Config) *TxPool {
	return &TxPool{
		...
		orphans:       make(map[chainhash.Hash]*abcutil.Tx),
		orphansByPrev: make(map[chainhash.Hash]map[chainhash.Hash]*abcutil.Tx),
		outpoints:     make(map[wire.OutPoint]*abcutil.Tx),
