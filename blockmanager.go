...
import (
	...
	"github.com/abcsuite/abcd/mempool"
...
// blockMsg packages a abcd block message and the peer it came from together
...
// invMsg packages a abcd inv message and the peer it came from together
// so the block handler has access to that information.
type invMsg struct {
	inv  *wire.MsgInv
	peer *serverPeer
...
// headersMsg packages a abcd headers message and the peer it came from
...
// txMsg packages a abcd tx message and the peer it came from together
// so the block handler has access to that information.
type txMsg struct {
	tx   *abcutil.Tx
	peer *serverPeer
...
func (b *blockManager) logBlockHeight(block *abcutil.Block) {
	b.receivedLogBlocks++
...
func (b *blockManager) checkBlockForHiddenVotes(block *abcutil.Block) {
	var votesFromBlock []*abcutil.Tx
	...
	var newVotes []*abcutil.Tx
	var oldTickets []*abcutil.Tx
	var oldRevocations []*abcutil.Tx
	oldVoteMap := make(map[chainhash.Hash]struct{},
		int(b.server.chainParams.TicketsPerBlock))
	if template != nil {
		templateBlock := abcutil.NewBlock(template.Block)
	...
	updatedTxTreeStake := make([]*abcutil.Tx, 0,
	...
	var updatedTxTreeRegular []*abcutil.Tx
...
func (b *blockManager) handleNotifyMsg(notification *blockchain.Notification) {
	...
	case blockchain.NTBlockConnected:
		blockSlice, ok := notification.Data.([]*abcutil.Block)
		if !ok {
		...
		// below.
		txTreeRegularValid := abcutil.IsFlagSet16(block.MsgBlock().Header.VoteBits,
			abcutil.BlockValid)
...
// queue.
func (b *blockManager) QueueTx(tx *abcutil.Tx, sp *serverPeer) {
...
func (b *blockManager) QueueBlock(block *abcutil.Block, sp *serverPeer) {
...
func (b *blockManager) CheckConnectBlock(block *abcutil.Block) error {
...
func (b *blockManager) GetBlockFromHash(h chainhash.Hash) (*abcutil.Block, error) {
...
func (b *blockManager) GetTopBlockFromChain() (*abcutil.Block, error) {
...
func (b *blockManager) ProcessBlock(block *abcutil.Block, flags blockchain.BehaviorFlags) (bool, error) {
...
func (b *blockManager) ProcessTransaction(tx *abcutil.Tx, allowOrphans bool,
	rateLimit bool, allowHighFees bool) ([]*abcutil.Tx, error) {
...
// pool.
func (b *blockManager) TicketPoolValue() (abcutil.Amount, error) {
...
// newBlockManager returns a new abcd block manager.
...
func warnMultipleDBs() {
	...
	if len(duplicateDbPaths) > 0 {
		...
		abcdLog.Warnf("WARNING: There are multiple block chain databases "+
...
func loadBlockDB() (database.DB, error) {
	...
	if cfg.DbType == "memdb" {
		abcdLog.Infof("Creating block database in memory.")
	...
	abcdLog.Infof("Loading block database from '%s'", dbPath)
	...
	abcdLog.Info("Block database loaded")
	return db, nil
}
