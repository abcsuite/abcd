...
const (
	// svcName is the name of abcd service.
	svcName = "abcdsvc"
	...
	svcDisplayName = "abcd Service"

	// svcDesc is the description of the service.
	svcDesc = "Downloads and stays synchronized with the abcd block " +
...
// logServiceStartOfDay logs information about abcd when the main server has
...
// abcdService houses the main service handler which handles all service
// updates and launching abcdMain.
...
// long-running abcdMain (which is the real meat of abcd), handles service
// change requests, and notifies the service control manager of changes.
func (s *abcdService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start abcdMain in a separate goroutine so the service can start
...
// installService attempts to install the abcd service.  Typically this should
...
// removeService attempts to uninstall the abcd service.  Typically this should
...
// startService attempts to start the abcd service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &abcdService{})
	if err != nil {
