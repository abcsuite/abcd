...
// processing.
func (m *wsNotificationManager) NotifyBlockConnected(block *abcutil.Block) {
...
func (m *wsNotificationManager) NotifyBlockDisconnected(block *abcutil.Block) {
...
func (m *wsNotificationManager) NotifyMempoolTx(tx *abcutil.Tx, isNew bool) {
...
func (f *wsClientFilter) addAddress(a abcutil.Address) {
	switch a := a.(type) {
...
func (f *wsClientFilter) addAddressStr(s string) {
	a, err := abcutil.DecodeAddress(s, activeNetParams.Params)
...
func (f *wsClientFilter) existsAddress(a abcutil.Address) bool {
	switch a := a.(type) {
	case *abcutil.AddressPubKeyHash:
...
func (f *wsClientFilter) removeAddress(a abcutil.Address) {
	switch a := a.(type) {
	case *abcutil.AddressPubKeyHash:
...
// Notification types
type notificationBlockConnected abcutil.Block
type notificationBlockDisconnected abcutil.Block
...
func (m *wsNotificationManager) subscribedClients(tx *abcutil.Tx,
...
func (m *wsNotificationManager) notifyBlockConnected(clients map[chan struct{}]*wsClient,
	...
	ntfn := abcjson.BlockConnectedNtfn{
	...
	for quitChan, client := range clients {
		...
		marshalledJSON, err := abcjson.MarshalCmd(nil, &ntfn)
		if err != nil {
...
// reorganize).
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *abcutil.Block) {
	...
	ntfn := abcjson.BlockDisconnectedNtfn{
		Header: hex.EncodeToString(headerBytes),
	}
	marshalledJSON, err := abcjson.MarshalCmd(nil, &ntfn)
	if err != nil {
...
func (m *wsNotificationManager) notifyReorganization(clients map[chan struct{}]*wsClient, rd *blockchain.ReorganizationNtfnsData) {
	...
	// Notify interested websocket clients about the disconnected block.
	ntfn := abcjson.NewReorganizationNtfn(rd.OldHash.String(),
		int32(rd.OldHeight),
		rd.NewHash.String(),
		int32(rd.NewHeight))
	marshalledJSON, err := abcjson.MarshalCmd(nil, ntfn)
...
func (*wsNotificationManager) notifyWinningTickets(
	...
	ntfn := abcjson.NewWinningTicketsNtfn(wtnd.BlockHash.String(),
...
func (*wsNotificationManager) notifySpentAndMissedTickets(
	...
	// Notify interested websocket clients about the connected block.
	ntfn := abcjson.NewSpentAndMissedTicketsNtfn(tnd.Hash.String(),
...
func (*wsNotificationManager) notifyNewTickets(clients map[chan struct{}]*wsClient,
	...
	// Notify interested websocket clients about the connected block.
	ntfn := abcjson.NewNewTicketsNtfn(tnd.Hash.String(), int32(tnd.Height),
...
func (*wsNotificationManager) notifyStakeDifficulty(
	...
	// Notify interested websocket clients about the connected block.
	ntfn := abcjson.NewStakeDifficultyNtfn(sdnd.BlockHash.String(),
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *abcutil.Tx) {
	...
	ntfn := abcjson.NewTxAcceptedNtfn(txHashStr,
		abcutil.Amount(amount).ToCoin())
	marshalledJSON, err := abcjson.MarshalCmd(nil, ntfn)
	if err != nil {
	...
	var verboseNtfn *abcjson.TxAcceptedVerboseNtfn
	...
	for _, wsc := range clients {
		if wsc.verboseTxUpdates {
			...
			verboseNtfn = abcjson.NewTxAcceptedVerboseNtfn(*rawTx)
			marshalledJSONVerbose, err = abcjson.MarshalCmd(nil,
				verboseNtfn)
			if err != nil {
...
// notifications.
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *abcutil.Tx,
	clients map[chan struct{}]*wsClient) {
	...
	if len(clientsToNotify) != 0 {
		n := abcjson.NewRelevantTxAcceptedNtfn(txHexString(msgTx))
		marshalled, err := abcjson.MarshalCmd(nil, n)
		if err != nil {
...
out:
	for {
		...
		switch authCmd, ok := cmd.cmd.(*abcjson.AuthenticateCmd); {
		...
		if !c.isAdmin {
			if _, ok := rpcLimited[request.Method]; !ok {
				jsonErr := &abcjson.RPCError{
					Code:    abcjson.ErrRPCInvalidParams.Code,
...
func handleWebsocketHelp(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*abcjson.HelpCmd)
	if !ok {
		return nil, abcjson.ErrRPCInternal
	}
...
func handleLoadTxFilter(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*abcjson.LoadTxFilterCmd)
...
func handleSession(wsc *wsClient, icmd interface{}) (interface{}, error) {
	return &abcjson.SessionResult{SessionID: wsc.sessionID}, nil
}
...
func handleNotifyNewTransactions(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*abcjson.NotifyNewTransactionsCmd)
	if !ok {
		return nil, abcjson.ErrRPCInternal
	}
...
func checkAddressValidity(addrs []string) error {
	for _, addr := range addrs {
		_, err := abcutil.DecodeAddress(addr, activeNetParams.Params)
		if err != nil {
...
func deserializeOutpoints(serializedOuts []abcjson.OutPoint) ([]*wire.OutPoint, error) {
...
// string slice.
func rescanBlock(filter *wsClientFilter, block *abcutil.Block) []string {
...
func handleRescan(wsc *wsClient, icmd interface{}) (interface{}, error) {
	...
	if filter == nil {
		return nil, &abcjson.RPCError{
			Code:    abcjson.ErrRPCMisc,
			Message: "Transaction filter must be loaded before rescanning",
		}
	}

	blockHashes, err := abcjson.DecodeConcatenatedHashes(cmd.BlockHashes)
	if err != nil {
		return nil, err
	}

	discoveredData := make([]abcjson.RescannedBlock, 0, len(blockHashes))
	...
	for i := range blockHashes {
		...
		if err != nil {
			return nil, &abcjson.RPCError{
				Code:    abcjson.ErrRPCBlockNotFound,
		...
		if len(transactions) != 0 {
			discoveredData = append(discoveredData, abcjson.RescannedBlock{
	...
	return &abcjson.RescanResult{DiscoveredData: discoveredData}, nil
}
