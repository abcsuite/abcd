...
func main() {
	...
	method := args[0]
	usageFlags, err := abcjson.MethodUsageFlags(method)
	if err != nil {
	...
	cmd, err := abcjson.NewCmd(method, params...)
	if err != nil {
		// Show the error along with its error code when it's a
		// abcjson.Error as it reallistcally will always be since the
		// NewCmd function is only supposed to return errors of that
		// type.
		if jerr, ok := err.(abcjson.Error); ok {
		...
		// The error is not a abcjson.Error and this really should not
	...
	marshalledJSON, err := abcjson.MarshalCmd(1, cmd)
	if err != nil {
