...
const (
	...
	unusableFlags = abcjson.UFWebsocketOnly | abcjson.UFNotification
)
...
func listCommands() {
	...
	cmdMethods := abcjson.RegisteredCmdMethods()
	categorized := make([][]string, numCategories)
	for _, method := range cmdMethods {
		flags, err := abcjson.MethodUsageFlags(method)
		if err != nil {
		...
		usage, err := abcjson.MethodUsageText(method)
		if err != nil {
			// This should never happen since the method was just
			// returned from the package, but be safe.
			continue
		...
		if flags&abcjson.UFWalletOnly != 0 {
...
// config defines the configuration options for abcctl.
//
...
// For this it tries to read the abcd config file at its default path, and extract
// the RPC user and password from it.
func createDefaultConfigFile(destinationPath string) error {
	// Nothing to do when there is no existing abcd conf file at the default
	// path to extract the details from.
	abcdConfigPath := filepath.Join(abcdHomeDir, "abcd.conf")
	if !fileExists(abcdConfigPath) {
		return nil
	}

	// Read abcd.conf from its default path
	abcdConfigFile, err := os.Open(abcdConfigPath)
	if err != nil {
		return err
	}
	defer abcdConfigFile.Close()
	content, err := ioutil.ReadAll(abcdConfigFile)
	if err != nil {
		return err
	}
