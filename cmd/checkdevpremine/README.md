...
It works by using the abcd RPC server to request all of the relevant input
...
In order to connect and authenticate to the abcd RPC server, the `--rpcuser` and
...
```
rpcuser=your_abcd_RPC_server_username
rpcpass=your_abcd_RPC_server_password
```
...
```bash
$ abcctl getbestblockhash | abcctl getblock - | jq -c .tx | checkdevpremine --quiet -; echo $?
0
```
...
```bash
$ go get -u github.com/abcsuite/abcd/cmd/checkdevpremine
```
