...
func traceDevPremineOuts(client *abcrpcclient.Client, txHash *chainhash.Hash) ([]wire.OutPoint, error) {
...
func realMain() int {
	...
	// Connect to abcd RPC server using websockets.
	...
	client, err := abcrpcclient.New(connCfg, nil)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Unable to connect to abcd RPC server: "+
			"%v\n", err)
		return rcError
	}
