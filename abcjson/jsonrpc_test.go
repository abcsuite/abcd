...
func TestIsValidIDType(t *testing.T) {
	...
	for i, test := range tests {
		if abcjson.IsValidIDType(test.id) != test.isValid {
...
func TestMarshalResponse(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			result: nil,
			jsonErr: func() *abcjson.RPCError {
				return abcjson.NewRPCError(abcjson.ErrRPCBlockNotFound, "123 not found")
			}(),
	...
	for i, test := range tests {
		_, _ = i, test
		marshalled, err := abcjson.MarshalResponse(testID, test.result, test.jsonErr)
		if err != nil {
...
func TestMiscErrors(t *testing.T) {
	...
	// not supported.
	_, err := abcjson.NewRequest(nil, "test", []interface{}{make(chan int)})
	if err == nil {
	...
	// supported.
	wantErr := abcjson.Error{Code: abcjson.ErrInvalidType}
	_, err = abcjson.MarshalResponse(make(chan int), nil, nil)
	if jerr, ok := err.(abcjson.Error); !ok || jerr.Code != wantErr.Code {
	...
	// can't be marshalled.
	_, err = abcjson.MarshalResponse(1, make(chan int), nil)
