...
func TestEncodeConcatenatedHashes(t *testing.T) {
	// Input Hash slice. Data taken from Aero's first three mainnet blocks.
	...
	for j := 0; j < len(hashSlice)+1; j++ {
		...
		// Encode to string
		concatenated := abcjson.EncodeConcatenatedHashes(hashSlice[:j])
		// Verify output
...
func TestDecodeConcatenatedHashes(t *testing.T) {
	// Test data taken from Aero's first three mainnet blocks
	...
	decodedHashes, err := abcjson.DecodeConcatenatedHashes(concatenatedHashes)
	if err != nil {
...
func TestEncodeConcatenatedVoteBits(t *testing.T) {
	...
	encodedResults, err := abcjson.EncodeConcatenatedVoteBits(testVbs)
	if err != nil {
	...
	_, err = abcjson.EncodeConcatenatedVoteBits(testVbs)
	if err == nil {
...
func TestDecodeConcatenatedVoteBits(t *testing.T) {
	...
	decodedSlice, err :=
		abcjson.DecodeConcatenatedVoteBits(encodedBytesStr)
	if err != nil {
	...
	decodedSlice, err = abcjson.DecodeConcatenatedVoteBits(encodedBytesStr)
	if err == nil {
