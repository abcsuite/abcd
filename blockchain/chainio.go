...
import (
	...
	"github.com/abcsuite/abcd/blockchain/internal/dbnamespace"
...
func serializeSizeForMinimalOutputs(tx *abcutil.Tx) int {
...
func putTxToMinimalOutputs(target []byte, tx *abcutil.Tx) int {
...
// The amount is never encoded into spent transaction outputs in Aero
...
func decodeSpentTxOut(serialized []byte, stxo *spentTxOut, amount int64,
	...
	// since in Aero we only need pkScript at most due to fraud proofs
...
// txouts.
func dbFetchSpendJournalEntry(dbTx database.Tx, block *abcutil.Block,
	parent *abcutil.Block) ([]spentTxOut, error) {
...
func (b *BlockChain) createChainState() error {
	...
	genesisBlock := abcutil.NewBlock(b.chainParams.GenesisBlock)
...
// from the index, and return a abcutil.Block with the height set.
func dbFetchBlockByHash(dbTx database.Tx, hash *chainhash.Hash) (*abcutil.Block, error) {
	...
	block, err := abcutil.NewBlockFromBytes(blockBytes)
	if err != nil {
		return nil, err
	}
...
// raw block for the provided height, deserialize it, and return a abcutil.Block
// with the height set.
func dbFetchBlockByHeight(dbTx database.Tx, height int64) (*abcutil.Block, error) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) BlockByHeight(blockHeight int64) (*abcutil.Block, error) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) BlockByHash(hash *chainhash.Hash) (*abcutil.Block, error) {
	b.chainLock.RLock()
