...
func AddrFromSStxPkScrCommitment(pkScript []byte,
	params *chaincfg.Params) (abcutil.Address, error) {
	...
	if isP2SH {
		addr, err = abcutil.NewAddressScriptHashFromHash(hashBytes, params)
	} else {
		addr, err = abcutil.NewAddressPubKeyHash(hashBytes, params,
...
// ticket commitment pkScript.
func AmountFromSStxPkScrCommitment(pkScript []byte) (abcutil.Amount, error) {
	if len(pkScript) < SStxPKHMinOutSize {
	...
	return abcutil.Amount(binary.LittleEndian.Uint64(amtEncoded)), nil
}
