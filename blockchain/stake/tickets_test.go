...
import (
	...
	"github.com/abcsuite/abcd/blockchain/stake/internal/tickettreap"
...
func ticketsInBlock(bl *abcutil.Block) []chainhash.Hash {
...
func ticketsSpentInBlock(bl *abcutil.Block) []chainhash.Hash {
	tickets := make([]chainhash.Hash, 0)
	for _, stx := range bl.STransactions() {
...
func votesInBlock(bl *abcutil.Block) []chainhash.Hash {
...
func revokedTicketsInBlock(bl *abcutil.Block) []chainhash.Hash {
	tickets := make([]chainhash.Hash, 0)
	for _, stx := range bl.STransactions() {
...
func TestTicketDBLongChain(t *testing.T) {
	...
	testBlockchain := make(map[int64]*abcutil.Block, len(testBlockchainBytes))
	for k, v := range testBlockchainBytes {
		bl, err := abcutil.NewBlockFromBytes(v)
		if err != nil {
