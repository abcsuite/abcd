...
import (
	...
	"github.com/abcsuite/abcd/blockchain/internal/progresslog"
...
func dbIndexConnectBlock(dbTx database.Tx, indexer Indexer, block, parent *abcutil.Block, view *blockchain.UtxoViewpoint) error {
...
func dbIndexDisconnectBlock(dbTx database.Tx, indexer Indexer, block, parent *abcutil.Block, view *blockchain.UtxoViewpoint) error {
...
func (m *Manager) Init(chain *blockchain.BlockChain) error {
	...
	var cachedParent *abcutil.Block
	for height := lowestHeight + 1; height <= bestHeight; height++ {
		var block, parent *abcutil.Block
		err = m.db.Update(func(dbTx database.Tx) error {
...
func makeUtxoView(dbTx database.Tx, block, parent *abcutil.Block) (*blockchain.UtxoViewpoint, error) {
	...
	if regularTxTreeValid {
		for txIdx, tx := range parent.Transactions() {
			...
			for _, txIn := range tx.MsgTx().TxIn {
				...
				view.AddTxOuts(abcutil.NewTx(originTx),
	...
	for _, tx := range block.STransactions() {
		...
		for i, txIn := range msgTx.TxIn {
			...
			view.AddTxOuts(abcutil.NewTx(originTx), int64(wire.NullBlockHeight),
				wire.NullBlockIndex)
		}
	}
...
//
// This is part of the blockchain.IndexManager interface.
func (m *Manager) ConnectBlock(dbTx database.Tx, block, parent *abcutil.Block, view *blockchain.UtxoViewpoint) error {
...
//
// This is part of the blockchain.IndexManager interface.
func (m *Manager) DisconnectBlock(dbTx database.Tx, block, parent *abcutil.Block, view *blockchain.UtxoViewpoint) error {
	// Call each of the currently active optional indexes with the block
