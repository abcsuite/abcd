...
func IsCoinBase(tx *abcutil.Tx) bool {
...
func CheckTransactionSanity(tx *wire.MsgTx, params *chaincfg.Params) error {
	...
	// known as an atom.  One abcd is a quantity of atoms as defined by
	...
	for _, txOut := range tx.TxOut {
		...
		// is detected and reported.  This is impossible for Aero,
		...
		if totalAtom > abcutil.MaxAmount {
			str := fmt.Sprintf("total value of all transaction "+
...
func checkProofOfStake(block *abcutil.Block, posLimit int64) error {
...
// difficulty is in min/max range and that the block hash is less than the
// target difficulty as claimed.
func CheckProofOfWork(block *abcutil.Block, powLimit *big.Int) error {
...
func checkBlockHeaderSanity(block *abcutil.Block, timeSource MedianTimeSource, flags BehaviorFlags, chainParams *chaincfg.Params) error {
...
func checkBlockSanity(block *abcutil.Block, timeSource MedianTimeSource, flags BehaviorFlags, chainParams *chaincfg.Params) error {
...
// sane before continuing with block processing.  These checks are context
// free.
func CheckBlockSanity(block *abcutil.Block, timeSource MedianTimeSource, chainParams *chaincfg.Params) error {
...
// context free.
func CheckWorklessBlockSanity(block *abcutil.Block, timeSource MedianTimeSource, chainParams *chaincfg.Params) error {
...
//
// Aero: Check the stake transactions to make sure they don't have this txid
// too.
func (b *BlockChain) checkDupTxs(txSet []*abcutil.Tx, view *UtxoViewpoint) error {
...
func (b *BlockChain) CheckBlockStakeSanity(stakeValidationHeight int64, node *blockNode, block *abcutil.Block, parent *abcutil.Block, chainParams *chaincfg.Params) error {
	...
	txTreeRegularValid := abcutil.IsFlagSet16(msgBlock.Header.VoteBits,
		abcutil.BlockValid)
	...
	for _, staketx := range stakeTransactions {
		...
		if is, _ := stake.IsSSGen(msgTx); is {
			...
			if abcutil.IsFlagSet16(ssGenVoteBits, abcutil.BlockValid) {
				voteYea++
			} else {
				voteNay++
			}
...
// owner of the abcd and therefore allowed to spend them.  As it checks the
// inputs, it also calculates the total fees for the transaction and returns
// that value.
//
// NOTE: The transaction MUST have already been sanity checked with the
// CheckTransactionSanity function prior to calling this function.
func CheckTransactionInputs(subsidyCache *SubsidyCache, tx *abcutil.Tx, txHeight int64, utxoView *UtxoViewpoint, checkFraudProof bool, chainParams *chaincfg.Params) (int64, error) {
	...
	// -------------------------------------------------------------------
	// Aero stake transaction testing.
	// -------------------------------------------------------------------
	...
	// -------------------------------------------------------------------
	// Aero general transaction testing (and a few stake exceptions).
	// -------------------------------------------------------------------
	for idx, txIn := range msgTx.TxIn {
		...
		// abcd is a quantity of atoms as defined by the AtomPerCoin
		// constant.
		...
		if originTxAtom > abcutil.MaxAmount {
			str := fmt.Sprintf("transaction output value of %v is "+
				"higher than max allowed value of %v",
				originTxAtom, abcutil.MaxAmount)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
...
// txscript.
func CountSigOps(tx *abcutil.Tx, isCoinBaseTx bool, isSSGen bool) int {
	msgTx := tx.MsgTx()
...
func CountP2SHSigOps(tx *abcutil.Tx, isCoinBaseTx bool, isStakeBaseTx bool, utxoView *UtxoViewpoint) (int, error) {
...
func checkNumSigOps(tx *abcutil.Tx, utxoView *UtxoViewpoint, index int, txTree bool, cumulativeSigOps int) (int, error) {
	msgTx := tx.MsgTx()
	isSSGen, _ := stake.IsSSGen(msgTx)
...
func checkStakeBaseAmounts(subsidyCache *SubsidyCache, height int64, params *chaincfg.Params, txs []*abcutil.Tx, utxoView *UtxoViewpoint) error {
	for _, tx := range txs {
		msgTx := tx.MsgTx()
		if is, _ := stake.IsSSGen(msgTx); is {
			// Ensure the input is available.
...
func getStakeBaseAmounts(txs []*abcutil.Tx, utxoView *UtxoViewpoint) (int64, error) {
	totalInputs := int64(0)
	totalOutputs := int64(0)
	for _, tx := range txs {
		msgTx := tx.MsgTx()
...
func getStakeTreeFees(subsidyCache *SubsidyCache, height int64, params *chaincfg.Params, txs []*abcutil.Tx, utxoView *UtxoViewpoint) (abcutil.Amount, error) {
	totalInputs := int64(0)
	totalOutputs := int64(0)
	for _, tx := range txs {
		msgTx := tx.MsgTx()
	...
	return abcutil.Amount(totalInputs - totalOutputs), nil
}
...
func (b *BlockChain) checkTransactionsAndConnect(subsidyCache *SubsidyCache, inputFees abcutil.Amount, node *blockNode, txs []*abcutil.Tx, utxoView *UtxoViewpoint, stxos *[]spentTxOut, txTree bool) error {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkConnectBlock(node *blockNode, block *abcutil.Block, utxoView *UtxoViewpoint, stxos *[]spentTxOut) error {
	...
	regularTxTreeValid := abcutil.IsFlagSet16(node.header.VoteBits,
		abcutil.BlockValid)
...
func (b *BlockChain) CheckConnectBlock(block *abcutil.Block) error {
	b.chainLock.Lock()
