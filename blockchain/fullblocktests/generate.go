...
import (
	...
	"github.com/abcsuite/abcd/blockchain/chaingen"
...
// subsidy.
func additionalCoinbasePoW(amount abcutil.Amount) func(*wire.MsgBlock) {
...
func additionalCoinbaseDev(amount abcutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
		// Increase the first proof-of-work coinbase subsidy by the
		// provided amount.
...
func additionalSpendFee(fee abcutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
func Generate() (tests [][]TestInstance, err error) {
	...
	g.NextBlock("bbadtaxscript", outs[5], ticketOuts[5], func(b *wire.MsgBlock) {
		...
		p2shTaxAddr := addrs[0].(*abcutil.AddressScriptHash)
		p2pkhTaxAddr, err := abcutil.NewAddressPubKeyHash(
	...
	g.NextBlock("b71", outs[15], ticketOuts[15], func(b *wire.MsgBlock) {
		b.Transactions[1].TxOut[0].Value = abcutil.MaxAmount + 1
	})
	rejected(blockchain.ErrBadTxOutValue)
	...
	g.NextBlock("b72", outs[15], ticketOuts[15], func(b *wire.MsgBlock) {
		b.Transactions[1].TxOut[0].Value = abcutil.MaxAmount
