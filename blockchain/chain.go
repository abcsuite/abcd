...
// BlockChain provides functions for working with the Aero block chain.
...
// StakeVersions is a condensed form of a abcutil.Block that is used to prevent
...
// exceeded.
func (b *BlockChain) addOrphanBlock(block *abcutil.Block) {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) getPrevNodeFromBlock(block *abcutil.Block) (*blockNode,
	error) {
	// Genesis block.
	prevHash := &block.MsgBlock().Header.PrevBlock
...
func (b *BlockChain) fetchBlockFromHash(hash *chainhash.Hash) (*abcutil.Block,
	error) {
	...
	var blockMainchain *abcutil.Block
...
func (b *BlockChain) GetTopBlock() (*abcutil.Block, error) {
...
func (b *BlockChain) pushMainChainBlockCache(block *abcutil.Block) {
...
// already there.
func dbMaybeStoreBlock(dbTx database.Tx, block *abcutil.Block) error {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBlock(node *blockNode, block *abcutil.Block,
	...
	blockAndParent := []*abcutil.Block{block, parent}
...
func (b *BlockChain) dropMainChainBlockCache(block *abcutil.Block) {
	curHash := block.Hash()
	b.mainchainBlockCacheLock.Lock()
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) disconnectBlock(node *blockNode, block *abcutil.Block,
...
func countSpentOutputs(block *abcutil.Block, parent *abcutil.Block) int {
	// We need to skip the regular tx tree if it's not valid.
	// We also exclude the coinbase transaction since it can't
	// spend anything.
	regularTxTreeValid := abcutil.IsFlagSet16(block.MsgBlock().Header.VoteBits,
		abcutil.BlockValid)
	var numSpent int
...
// adding the block.
func countNumberOfTransactions(block, parent *abcutil.Block) uint64 {
	var numTxns uint64
...
func (b *BlockChain) reorganizeChain(detachNodes, attachNodes *list.List,
	...
	detachBlocks := make([]*abcutil.Block, 0, detachNodes.Len())
	...
	for e := detachNodes.Front(); e != nil; e = e.Next() {
		n := e.Value.(*blockNode)
		var block *abcutil.Block
		var parent *abcutil.Block
		var err error
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBestChain(node *blockNode, block *abcutil.Block,
	...
	if node.header.PrevBlock == b.bestNode.hash {
		...
		txTreeRegularValid := abcutil.IsFlagSet16(node.header.VoteBits,
			abcutil.BlockValid)
...
type IndexManager interface {
	...
	// main chain.
	ConnectBlock(database.Tx, *abcutil.Block, *abcutil.Block, *UtxoViewpoint) error

	// DisconnectBlock is invoked when a block has been disconnected from
	// the main chain.
	DisconnectBlock(database.Tx, *abcutil.Block, *abcutil.Block, *UtxoViewpoint) error
}
...
func New(config *Config) (*BlockChain, error) {
	...
	b := BlockChain{
		...
		blockCache:                    make(map[chainhash.Hash]*abcutil.Block),
		mainchainBlockCache:           make(map[chainhash.Hash]*abcutil.Block),
