...
func (s *SpendableOut) Amount() abcutil.Amount {
	return s.amount
}
...
func MakeSpendableOutForTx(tx *wire.MsgTx, blockHeight, txIndex, txOutIndex uint32) SpendableOut {
	return SpendableOut{
		...
		blockIndex:  txIndex,
		amount:      abcutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
type Generator struct {
	...
	p2shOpTrueAddr   abcutil.Address
...
func MakeGenerator(params *chaincfg.Params) (Generator, error) {
	...
	p2shOpTrueAddr, err := abcutil.NewAddressScriptHash(opTrueScript, params)
	if err != nil {
...
func (g *Generator) calcFullSubsidy(blockHeight uint32) abcutil.Amount {
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcPoWSubsidy(fullSubsidy abcutil.Amount, blockHeight uint32, numVotes uint16) abcutil.Amount {
	powProportion := abcutil.Amount(g.params.WorkRewardProportion)
	totalProportions := abcutil.Amount(g.params.TotalSubsidyProportions())
	...
	ticketsPerBlock := abcutil.Amount(g.params.TicketsPerBlock)
	return (powSubsidy * abcutil.Amount(numVotes)) / ticketsPerBlock
}
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcPoSSubsidy(heightVotedOn uint32) abcutil.Amount {
	if int64(heightVotedOn+1) < g.params.StakeValidationHeight {
		return 0
	}

	fullSubsidy := g.calcFullSubsidy(heightVotedOn)
	posProportion := abcutil.Amount(g.params.StakeRewardProportion)
	totalProportions := abcutil.Amount(g.params.TotalSubsidyProportions())
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcDevSubsidy(fullSubsidy abcutil.Amount, blockHeight uint32, numVotes uint16) abcutil.Amount {
	devProportion := abcutil.Amount(g.params.BlockTaxProportion)
	totalProportions := abcutil.Amount(g.params.TotalSubsidyProportions())
	...
	// Reduce the subsidy according to the number of votes.
	ticketsPerBlock := abcutil.Amount(g.params.TicketsPerBlock)
	return (devSubsidy * abcutil.Amount(numVotes)) / ticketsPerBlock
}
...
func (g *Generator) addCoinbaseTxOutputs(tx *wire.MsgTx, blockHeight uint32, devSubsidy, powSubsidy abcutil.Amount) {
...
func purchaseCommitmentScript(addr abcutil.Address, amount, voteFeeLimit, revocationFeeLimit abcutil.Amount) []byte {
...
func (g *Generator) createTicketPurchaseTx(spend *SpendableOut, ticketPrice, fee abcutil.Amount) *wire.MsgTx {
...
func (g *Generator) createVoteTx(parentBlock *wire.MsgBlock, ticket *stakeTicket) *wire.MsgTx {
	...
	voteSubsidy := posSubsidy / abcutil.Amount(g.params.TicketsPerBlock)
	ticketPrice := abcutil.Amount(ticket.tx.TxOut[0].Value)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	utilTxns := make([]*abcutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, abcutil.NewTx(tx))
	}
...
func (g *Generator) CreateSpendTx(spend *SpendableOut, fee abcutil.Amount) *wire.MsgTx {
...
// opTrueRedeemScript.
func (g *Generator) CreateSpendTxForTx(tx *wire.MsgTx, blockHeight, txIndex uint32, fee abcutil.Amount) *wire.MsgTx {
...
func (g *Generator) NextBlock(blockName string, spend *SpendableOut, ticketSpends []SpendableOut, mungers ...func(*wire.MsgBlock)) *wire.MsgBlock {
	...
	ticketPrice := abcutil.Amount(g.calcNextRequiredStakeDifficulty())
	...
	if nextHeight > uint32(g.params.CoinbaseMaturity) {
		...
		// outputs.
		if ticketSpends != nil {
			const ticketFee = abcutil.Amount(2)
	...
	var stakeTreeFees abcutil.Amount
	for txIdx, tx := range stakeTxns {
		...
		var inputSum, outputSum abcutil.Amount
		for _, txIn := range tx.TxIn {
			inputSum += abcutil.Amount(txIn.ValueIn)
		}
		for _, txOut := range tx.TxOut {
			outputSum += abcutil.Amount(txOut.Value)
		}
...
func (g *Generator) CreatePremineBlock(blockName string, additionalAmount abcutil.Amount) *wire.MsgBlock {
	...
	var totalSubsidy abcutil.Amount
	for _, payout := range g.params.BlockOneLedger {
		payoutAddr, err := abcutil.DecodeAddress(payout.Address, g.params)
		if err != nil {
			panic(err)
		}
		...
		totalSubsidy += abcutil.Amount(payout.Amount)
	}
