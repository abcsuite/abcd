...
// newAddressPubKey returns a new abcutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) abcutil.Address {
	addr, err := abcutil.NewAddressSecpPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new abcutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) abcutil.Address {
	addr, err := abcutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams,
		secp)
	if err != nil {
...
// newAddressScriptHash returns a new abcutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) abcutil.Address {
	addr, err := abcutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the abcutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the abcutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := abcutil.NewAddressPubKeyHash(decodeHex("e34cce70c863"+
	...
	p2shMain, _ := abcutil.NewAddressScriptHashFromHash(decodeHex("e8c300"+
	...
	p2pkCompressedMain, err := abcutil.NewAddressSecpPubKey(decodeHex("02192d74"+
	...
	p2pkCompressed2Main, err := abcutil.NewAddressSecpPubKey(decodeHex("03b0bd"+
	...
	p2pkUncompressedMain, err := abcutil.NewAddressSecpPubKey(decodeHex("0411db"+
	...
	tests := []struct {
		...
		// pay-to-pubkey address on mainnet. for abcd this would
		// be uncompressed, but standard for abcd is 33 byte
		...
		{(*abcutil.AddressPubKeyHash)(nil), "", txscript.ErrUnsupportedAddress},
		{(*abcutil.AddressScriptHash)(nil), "", txscript.ErrUnsupportedAddress},
		{(*abcutil.AddressSecpPubKey)(nil), "", txscript.ErrUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	//  mainnet p2pk 13CG6SJ3yHUXo4Cr2RY4THLLJrNFuG3gUg
	p2pkCompressedMain, err := abcutil.NewAddressSecpPubKey(decodeHex("02192d7"+
	...
	p2pkUncompressedMain, err := abcutil.NewAddressSecpPubKey(decodeHex("0411d"+
	...
	tests := []struct {
		keys      []*abcutil.AddressSecpPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*abcutil.AddressSecpPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
		...
		{
			// By default compressed pubkeys are used in Aero.
			[]*abcutil.AddressSecpPubKey{
				p2pkUncompressedMain,
			},
			1,
