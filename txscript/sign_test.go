...
func mkGetKey(keys map[string]addressToKey) txscript.KeyDB {
	if keys == nil {
		return txscript.KeyClosure(func(addr abcutil.Address) (chainec.PrivateKey,
			bool, error) {
...
func mkGetScript(scripts map[string][]byte) txscript.ScriptDB {
	if scripts == nil {
		return txscript.ScriptClosure(func(addr abcutil.Address) (
			[]byte, error) {
			return nil, errors.New("nope 3")
		})
	}
	return txscript.ScriptClosure(func(addr abcutil.Address) ([]byte,
		error) {
...
func TestSignTxOutput(t *testing.T) {
	...
	for _, hashType := range hashTypes {
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				address, err := abcutil.NewAddressPubKeyHash(
					abcutil.Hash160(pkBytes), testingParams,
					suite)
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			address, err := abcutil.NewAddressPubKeyHash(
				abcutil.Hash160(pkBytes), testingParams, secp)
			if err != nil {
				t.Errorf("failed to make address for %s: %v",
					msg, err)
				break
	...
	for _, hashType := range hashTypes {
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				address, err := abcutil.NewAddressSecpPubKey(pkBytes,
					testingParams)
				if err != nil {
					t.Errorf("failed to make address for %s: %v",
						msg, err)
		...
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				case edwards:
					...
					key, pk = chainec.Edwards.PrivKeyFromBytes(keyDB)
					pkBytes = pk.SerializeUncompressed()
					address, err = abcutil.NewAddressEdwardsPubKey(pkBytes,
						testingParams)
					if err != nil {
						t.Errorf("failed to make address for %s: %v",
							msg, err)
				...
				case secSchnorr:
					...
					key, pk = chainec.SecSchnorr.PrivKeyFromBytes(keyDB)
					pkBytes = pk.Serialize()
					address, err = abcutil.NewAddressSecSchnorrPubKey(pkBytes,
						testingParams)
					if err != nil {
						t.Errorf("failed to make address for %s: %v",
							msg, err)
	...
	for _, hashType := range hashTypes {
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				address, err := abcutil.NewAddressPubKeyHash(
					abcutil.Hash160(pkBytes), testingParams, suite)
				if err != nil {
					t.Errorf("failed to make address for %s: %v",
						msg, err)
					break
				...
				scriptAddr, err := abcutil.NewAddressScriptHash(
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			address1, err := abcutil.NewAddressSecpPubKey(pk1Bytes,
				testingParams)
			if err != nil {
				t.Errorf("failed to make address for %s: %v",
					msg, err)
			...
			address2, err := abcutil.NewAddressSecpPubKey(pk2Bytes,
				testingParams)
			if err != nil {
			...
			pkScript, err := txscript.MultiSigScript(
				[]*abcutil.AddressSecpPubKey{address1, address2},
				2)
			if err != nil {
				t.Errorf("failed to make pkscript "+
					"for %s: %v", msg, err)
...
var (
	...
	thisAddressUnc, _ = abcutil.NewAddressPubKeyHash(
		abcutil.Hash160(thisPubKey.SerializeUncompressed()),
		testingParams, secp)
	uncompressedPkScript, _ = txscript.PayToAddrScript(thisAddressUnc)
	thisAddressCom, _       = abcutil.NewAddressPubKeyHash(
		abcutil.Hash160(thisPubKey.SerializeCompressed()),
		testingParams, secp)
