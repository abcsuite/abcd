...
// valid SStxs.
func PayToSStx(addr abcutil.Address) ([]byte, error) {
	if addr == nil {
...
// valid SStxs.
func PayToSStxChange(addr abcutil.Address) ([]byte, error) {
	if addr == nil {
		return nil, ErrUnsupportedAddress
	}
...
// valid SSGen txs.
func PayToSSGen(addr abcutil.Address) ([]byte, error) {
	if addr == nil {
		return nil, ErrUnsupportedAddress
	}
...
// valid SSRtx.
func PayToSSRtx(addr abcutil.Address) ([]byte, error) {
	if addr == nil {
		return nil, ErrUnsupportedAddress
	}
...
// an SStx.
func GenerateSStxAddrPush(addr abcutil.Address, amount abcutil.Amount,
...
// specified address.
func PayToAddrScript(addr abcutil.Address) ([]byte, error) {
	switch addr := addr.(type) {
	case *abcutil.AddressPubKeyHash:
		if addr == nil {
			return nil, ErrUnsupportedAddress
...
func MultiSigScript(pubkeys []*abcutil.AddressSecpPubKey, nrequired int) ([]byte,
	error) {
...
func ExtractPkScriptAddrs(version uint16, pkScript []byte,
	chainParams *chaincfg.Params) (ScriptClass, []abcutil.Address, int, error) {
	if version != DefaultScriptVersion {
	...
	case PubKeyHashTy:
		...
		requiredSigs = 1
		addr, err := abcutil.NewAddressPubKeyHash(pops[2].data,
	...
	case PubKeyTy:
		...
		requiredSigs = 1
		addr, err := abcutil.NewAddressSecpPubKey(pops[0].data, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case PubkeyAltTy:
		...
		switch suite {
		case chainec.ECTypeEdwards:
			addr, err = abcutil.NewAddressEdwardsPubKey(pops[0].data,
				chainParams)
		case chainec.ECTypeSecSchnorr:
			addr, err = abcutil.NewAddressSecSchnorrPubKey(pops[0].data,
				chainParams)
		}
		if err == nil {
			addrs = append(addrs, addr)
	...
	case StakeSubmissionTy:
		...
		var localAddrs []abcutil.Address
	...
	case ScriptHashTy:
		...
		requiredSigs = 1
		addr, err := abcutil.NewAddressScriptHashFromHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case MultiSigTy:
		...
		addrs = make([]abcutil.Address, 0, numPubKeys)
		for i := 0; i < numPubKeys; i++ {
			addr, err := abcutil.NewAddressSecpPubKey(pops[i+1].data,
				chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
